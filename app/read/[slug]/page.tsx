...
import { Asterisk, ChevronLeft, Tag, UserRoundPen } from "lucide-react";
    ...
    <>
      <section className="mx-auto w-full">
        <div className="container my-8 flex items-center justify-between">
          <Link
            className="flex flex-row items-center text-xl no-underline"
            href={`/`}
          >
            <Button variant="link">
              {" "}
          ...
          <Link
            ...
            href={`/tags/${post.tags[0].trim().toLowerCase().replaceAll(" ", "-")}`}
          >
            <Button variant="link">
              {" "}
              <Tag /> {post.tags[0]}{" "}
      ...
      <article className="container mx-auto my-12 prose w-full max-w-4xl prose-stone lg:prose-xl dark:prose-invert">
        ...
        <div className="not-prose flex flex-wrap items-center justify-between">
          <Link
            href={`/author/${post.author.toLowerCase().trim().split(" ").join("-")}`}
          >
            <Button className="-px-2 text-xl" variant="link">
              {" "}
              Published By
              <UserRoundPen /> {post.author}{" "}
            </Button>
          </Link>
          ...
          <div className="flex flex-row items-center justify-between">
            <time
              ...
              title={dayjs(post.date).format("DD MMM YYYY")}
            >
              {dayjs(post.date).format("DD MMM YYYY")}
            </time>
            <div className="mx-1 h-1 w-1 rounded-full bg-black text-base dark:bg-white"></div>
        ...
        <p>
          ...
          Culpa aute laborum velit velit in do. Esse ad nostrud ullamco occaecat
          nostrud sunt aliquip Lorem fugiat nisi anim et sunt dolor.
        </p>
        <h2>
          {" "}
        ...
        <p>
          ...
          Lorem exercitation amet. Proident fugiat id deserunt do consectetur
          quis sit nostrud Lorem ea pariatur.
        </p>
        <p>
          {" "}
      ...
      <div className="my-10 flex w-full items-center rounded-full">
        ...
        <span className="flex flex-row px-8 py-3 text-lg leading-8 font-semibold"></span>
        <div className="border-fd-border flex-1 border-b"></div>
      </div>
      <Newsletter />
    </>
